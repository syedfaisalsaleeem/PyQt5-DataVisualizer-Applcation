...
def plot(
    ...
    **kwargs
    ...
    if 'color' not in text_kwargs:
        text_kwargs['color']=['k' for i in sizes]
    ...
    if value is not None:
        ...
        for v, r, color in zip(value, rects, text_kwargs['color']):
            x, y, dx, dy = r["x"], r["y"], r["dx"], r["dy"]
            ax.text(x + dx / 2, y + dy / 2, v, va=va, ha="center", color=color)

    if label is not None:
        va = "center" if value is None else "bottom"
        for l, r,  color in zip(label, rects,text_kwargs['color']):
            x, y, dx, dy = r["x"], r["y"], r["dx"], r["dy"]
            ax.text(x + dx / 2, y + dy / 2, l, va=va, ha="center", color=color)
