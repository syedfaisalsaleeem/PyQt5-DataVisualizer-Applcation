...
try:  # pragma: no cover
    ...
    def bubbles(circles, ax, color, texts, text_colors=None, labels=None, lim=None):
        """Debugging function displays circles with matplotlib."""
        if not labels:
            labels = [get_default_label(i, c) for i, c in enumerate(circles)]
        if not text_colors:
            text_colors = ['k' for i in circles]
        for circle, label, color, text, text_color in zip(circles, labels, color, texts, text_colors):
            x, y, r = circle.circle
            ax.add_patch(plt.Circle((x, y), r, facecolor=color))
            ax.text(x, y, str(text)+'\n'+str(label), ha='center', va='center',color=text_color)
        if lim is None:
            lim = max(
                max(
